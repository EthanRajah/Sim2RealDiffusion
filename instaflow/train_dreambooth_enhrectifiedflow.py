...
# Uniform dist for t is not as effective for diffusion sampling as it overweights midpoints and underweights near-0 and near-1
...
def main(args):
    ...
    exponential_dist = ExponentialPDF(a=0, b=1, name='ExponentialPDF') # Init the exponential distribution with bounds [0, 1]
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                bsz = latents.shape[0]
                # Sample a random timestep for each image (rectified flow: 0 to 1)
                # Sample from continuous distribution for better performance. Paper suggests a=4.0
                t = sample_t(exponential_dist, bsz, a=4.0).to(latents.device) # (batchsize,)
                t_expanded = t.view(-1, 1, 1, 1)

                # Rectified flow: linear interpolation x_t = (1-t) * x_0 + t * x_1, where x_0 = noise (source) and x_1 = clean latents (target)
                # noisy_latents spans the range from noise to clean latents. Useful for predicting the vector between the latents and noise
                # Forward (linear) diffusion process
                ...
                # Predict the velocity field using the UNet model across range of timesteps
                # Ensures model learns to denoise from any point along the forward diffusion process
                # Predict velocity vector between clean latents and noise
                model_pred = unet(noisy_latents, t, encoder_hidden_states).sample

                # Rectified flow target: velocity field v = x_1 - x_0 = clean latents - noise
                target = latents - noise

                if args.with_prior_preservation:
                    # Prior preservation loss: regularization to prevent overfitting on instance images
                    ...
                    # Prior preservation is not used
                    loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean")
