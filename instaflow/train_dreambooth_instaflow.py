# /home/ethan/DiffusionResearch/diffusion_env/bin/accelerate launch train_dreambooth_instaflow.py   --pretrained_model_name_or_path=$MODEL_NAME    --instance_data_dir=$INSTANCE_DIR   --output_dir=$OUTPUT_DIR   --train_text_encoder   --num
# _dataloader_workers=1   --instance_prompt="pushblock"   --resolution=512   --train_batch_size=4   --lr_scheduler="constant"   --lr_warmup_steps=0   --use_lora   --lora_r 16   -
# -lora_alpha 27   --lora_text_encoder_r 16   --lora_text_encoder_alpha 17   --learning_rate=1e-5   --gradient_accumulation_steps=1   --gradient_checkpointing --max_train_steps 1
# 500 --checkpointing_steps 250
...
def main(args):
    ...
    # Potentially load in the weights and states from a previous save BEFORE preparing with accelerator
    global_step = 0
    first_epoch = 0
    ...
    if args.resume_from_checkpoint:
        ...
        # Check if this is a PEFT adapter checkpoint or old accelerator checkpoint
        unet_adapter_path = os.path.join(checkpoint_path, "unet", "adapter_model.safetensors")
        te_adapter_path = os.path.join(checkpoint_path, "text_encoder", "adapter_model.safetensors")
        
        if args.use_lora and os.path.exists(unet_adapter_path):
            # Load PEFT adapter checkpoint
            accelerator.print("Loading PEFT adapter checkpoint")
            from peft import PeftModel
            
            # Load UNet adapter
            unet_checkpoint_path = os.path.join(checkpoint_path, "unet")
            unet = PeftModel.from_pretrained(unet, unet_checkpoint_path)
            
            # Load Text Encoder adapter if it exists
            if args.train_text_encoder and os.path.exists(te_adapter_path):
                te_checkpoint_path = os.path.join(checkpoint_path, "text_encoder")
                text_encoder = PeftModel.from_pretrained(text_encoder, te_checkpoint_path)
            
            # We need to manually set the global step for PEFT checkpoints
            global_step = int(path.split("-")[1])
        else:
            # For old accelerator checkpoint format, we'll load after prepare
            pass
    ...
    # Load old accelerator checkpoint after prepare if needed
    if args.resume_from_checkpoint:
        checkpoint_path = os.path.join(args.output_dir, path)
        unet_adapter_path = os.path.join(checkpoint_path, "unet", "adapter_model.safetensors")
        
        if not (args.use_lora and os.path.exists(unet_adapter_path)):
            # Load old accelerator checkpoint format
            accelerator.print("Loading accelerator checkpoint")
    ...
    # Calculate resume step if needed
    first_epoch = 0
    if args.resume_from_checkpoint:
        ...
        with TorchTracemalloc() if not args.no_tracemalloc else nullcontext() as tracemalloc:
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(unet):
                    ...
                    timesteps = (t * 1000).long()  # Scale t to match the scheduler's timesteps

                    # Rectified flow: linear interpolation x_t = (1-t) * x_0 + t * x_1
                    # Instead of the target being the noise added for the step, it's now the vector between the latent and the noise
                    # This allows us during inference to do X_1 = X_0 + v * t, where v is the learned ODE
                    ...
                    # Predict the ODE step using the UNet model
                ...
                if accelerator.sync_gradients:
                    ...
                    if global_step % args.checkpointing_steps == 0:
                        if accelerator.is_main_process:
                            ...
                            # Save in PEFT adapter format (same as final model)
                            if args.use_lora:
                                # Save UNet adapter
                                ...
                                # Save Text Encoder adapter if training text encoder
                                if args.train_text_encoder:
                                    unwarpped_text_encoder = accelerator.unwrap_model(text_encoder)
                                    te_save_path = os.path.join(save_path, "text_encoder")
                                ...
                                # Fallback to original checkpoint saving for non-LoRA training
